...
Test(SFML_Library, INVALID_EVENT_TYPE, .description = "Send invalid event type")
{
	...
	tmp.type = sf::Event::Closed;
	sfml->setEvent(tmp);
	cr_assert_not(sfml->GetKey(arcade::KEYBOARD, arcade::CLOSE), "Send invalid event type failed !\n");
}
...
Test(SFML_Library, INVALID_KEY_CODE, .description = "Send invalid key code")
{
	...
	tmp.type = sf::Event::Closed;
	sfml->setEvent(tmp);
	cr_assert_not(sfml->GetKey(arcade::WINDOW, "a"), "Send invalid key code failed !\n");
}
...
Test(SFML_LIBRARY, Load_SFML_LIBRARY)
{
	...
	std::unique_ptr<Loader>	loader(new Loader((char *)"./lib/Graphics/Sfml/lsfml_graphic.so"));
